 ...
 */
package ru.profi1c.samples.order.exchange;
...
import ru.profi1c.samples.order.db.DBHelper;
...
public class ExchangeManager extends BaseExchangeManager {
    ...
    public boolean startExchange(ExchangeVariant exchangeVariant) throws Exception {
        ...
        if (!isRunningExchangeTask()) {
            ExchangeTask task =
                    new ExchangeTask(exchangeVariant, getExchangeStrategy(), getDBOpenHelper());
            return task.call();
        }
        return false;
